...
export function Character(props: CharacterProps) {
  ...
  useEffect(() => {
    setInterval(() => {
      ...
      if (countRef.current >= faceResults.length - 1) {
        countRef.current = 0
      }
      updateBlendshapes(headMesh, nodes, faceResults[countRef.current].blendshapes, faceResults[countRef.current].transformationMatrices);
      // updatePose(poseResults.pose[countRef.current], poseResults.poseWorld[countRef.current], bones)
