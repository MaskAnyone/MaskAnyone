# Copied from https://github.com/Voice-Privacy-Challenge/Voice-Privacy-Challenge-2022/blob/master/baseline/local/pitch_correlation.py
...
def pitchCorr(wav_orig, wav_anon):
    ...
    if len(a) > len(b):
        b = interp1d(np.linspace(1, len(b), num=len(b)), b)(
            np.linspace(1, len(b), num=len(a))
        )
    elif len(a) < len(b):
        a = interp1d(np.linspace(1, len(a), num=len(a)), a)(
            np.linspace(1, len(a), num=len(b))
        )
    ...
    with ReadHelper(f"scp:{wav_list_orig}") as reader_orig:
        with ReadHelper(f"scp:{wav_list_anon}") as reader_anon:
            ...
            for utid_orig, (freq_orig, samp_orig) in reader_orig:
                ...
                utid_anon, (freq_anon, samp_anon) = next(
                    iter_anon, (None, (None, None))
                )
                assert (
                    utid_anon is not None
                ), f"Mismatch between lists of original and anonymized files: {wav_list_orig} and {wav_list_anon}"
                assert (
                    utid_anon == utid_orig
                ), f"Different order of utterances in the lists of original and anonymized files: {wav_list_orig} and {wav_list_anon}"
                assert (
                    freq_anon == freq_orig
                ), f"Different sampling frequency of original and anonymized files: {wav_list_orig} and {wav_list_anon}"
                assert (
                    freq_anon == 16000
                ), f"Wrong sampling frequency (should be 16000): {wav_list_orig} and {wav_list_anon}"
                diff = abs(len(samp_anon) - len(samp_orig)) / freq_orig * 1000  # in ms
                if diff > max_len_diff:
                    print(
                        f"Warning! Difference between lenghts of original and anonymized utterances is too long (exceeds #threshold max_len_diff={max_len_diff} ms): {utid_orig}  and {utid_anon}, difference = {diff} ms"
                    )
