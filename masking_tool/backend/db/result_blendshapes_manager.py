...
class ResultBlendshapesManager:
    ...
    def create_result_mp_kinematics_entry(self, id: str, result_video_id: str, video_id: str, job_id: str, data: dict):
        self.__db_connection.execute(
            ...
            {"id": id, "result_video_id": result_video_id, "video_id": video_id, "job_id": job_id, "data": json.dumps(data)},
        )

    def fetch_result_blendshapes_entry(self, blendshapes_id: str):
        result_blendshapes_data_list = self.__db_connection.select_all(
            "SELECT * FROM result_blendshapes WHERE id=%(id)s",
            {'id': blendshapes_id}
        )

        if len(result_blendshapes_data_list) < 1:
            raise Exception('Could not find blendshapes entry with id ' + blendshapes_id)
